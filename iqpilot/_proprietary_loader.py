...
_KONN3KT_API_HOST = os.environ.get("KONN3KT_API_HOST", "https://api-konn3kt.teallvbs.xyz").rstrip("/")
...
def _iter_proprietary_python_roots() -> list[Path]:
  ...
  # Signed artifact bundles (always present in release builds).
...
def _extend_private_package_paths(python_root: Path) -> None:
  for mod_name, mod in list(sys.modules.items()):
    if mod is None or not hasattr(mod, "__path__"):
      continue
    if mod_name != "iqpilot_private" and not mod_name.startswith("iqpilot_private."):
      continue
    subdir = python_root / mod_name.replace(".", "/")
    if subdir.is_dir():
      subdir_str = str(subdir)
      if subdir_str not in mod.__path__:
        mod.__path__.append(subdir_str)
...
def _ensure_private_path(private_module_name: str) -> None:
  ...
  if resolved_root is not None:
    ...
    _extend_private_package_paths(resolved_root)
    return
  ...
  for python_root in _iter_proprietary_python_roots():
    if (python_root / "iqpilot_private").exists():
      ...
      _extend_private_package_paths(python_root)
      return
