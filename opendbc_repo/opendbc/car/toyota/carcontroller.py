...
class CarController(CarControllerBase, GasInterceptorCarController):
  ...
  def update(self, CC, CC_IQ, CS, now_nanos):
    ...
    if (self.CP.carFingerprint not in NO_STOP_TIMER_CAR and not (self.CP_IQ.flags & ToyotaFlagsIQ.STOP_AND_GO_HACK)) or \
       self.CP_IQ.enableGasInterceptor:
      ...
      # NO_STOP_TIMER cars and SNG hack: actively manage standstill hold/release based on planner intent.
